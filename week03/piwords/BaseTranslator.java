...
public class BaseTranslator {
   ...
   * LEFT a. x = multiply the jth digit by baseB and add the carry b. the new jth
  ...
  public static int[] convertBase(int[] digits, int baseA, int baseB, int precisionB) {
    ...
    if ((baseA < 2) || (baseB < 2) || (precisionB < 1) || (Arrays.stream(digits).anyMatch(x -> x < 0))
        || (Arrays.stream(digits).anyMatch(x -> x >= baseA))) {
      return null;
    }
