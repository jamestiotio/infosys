/**
 * 6.005 Elements of Software Construction (c) 2007-8-9, MIT and Rob Miller
 */
package midi;
...
public class Midi {
    ...
    private final Map<midi.Instrument, MidiChannel> channels =
            new HashMap<midi.Instrument, MidiChannel>();
    ...
    /**
     * Returns a trace of events on this object since the last call to clearHistory(). Events are
     * formatted as follows: "on(pitch,instr)" for every note-on event. "off(pitch,instr)" for every
     * note-off event. "rest(time)" denotes the time separating every pair of note-on or note-off
     * events. In the parameters above, "pitch" is the midi frequency of the note event, "instr" is
     * the instrument name as it appears in the Instrument enum, and "time" is the separation time
     * in hundredths of a second. These individual event logs are space-separated.
     * 
     * As an example, suppose we are on the default piano instrument. We clearHistory(), then start
     * a note with midi frequency 60, then rest 30 ms, then start a note with midi frequency 65,
     * then rest 100 ms, then release the first note, then rest 30 ms, then release the second note,
     * and then call history(). history() will return this string: "on(60,PIANO) rest(3)
     * on(65,PIANO) rest(10) off(60,PIANO) rest(3) off(65,PIANO)"
     * 
     ...
     * 
     * We iterate through the channels of the synthesizer circularly, thereby returning the least
     * recently allocated channel.
     * 
     * @return channel
     */
    private MidiChannel allocateChannel() {
        ...
        if (nextChannel >= channels.length)
            throw new RuntimeException(
                    "tried to use too many instruments: limited to " + channels.length);
    ...
    /**
     * Discover all the instruments in your MIDI synthesizer and print them to standard output,
     * along with their bank and program codes.
     */
    public static void main(String[] args) throws MidiUnavailableException {
        Midi m = new Midi();
        for (javax.sound.midi.Instrument i : m.synthesizer.getLoadedInstruments()) {
            System.out.println(
                    i.getName() + " " + i.getPatch().getBank() + " " + i.getPatch().getProgram());
        }
    }
}
